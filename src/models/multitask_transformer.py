...
class MultiTaskDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        y_tensors = {
            target: torch.tensor(self.y_train[target], dtype=torch.float32)
            for target in self.target_names
            if target in self.y_train
        }
        mask_tensors = {
            target: torch.tensor(self.mask_train[target], dtype=torch.bool)
            for target in self.target_names
            if target in self.mask_train
        }
    ...
    def val_dataloader(self):
        ...
        y_tensors = {
            target: torch.tensor(self.y_val[target], dtype=torch.float32)
            for target in self.target_names
            if target in self.y_val
        }
        mask_tensors = {
            target: torch.tensor(self.mask_val[target], dtype=torch.bool)
            for target in self.target_names
            if target in self.mask_val
        }
    ...
    def test_dataloader(self):
        ...
        y_tensors = {
            target: torch.tensor(self.y_test[target], dtype=torch.float32)
            for target in self.target_names
            if target in self.y_test
        }
        mask_tensors = {
            target: torch.tensor(self.mask_test[target], dtype=torch.bool)
            for target in self.target_names
            if target in self.mask_test
        }
...
class MultiTaskDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self, X: torch.Tensor, y_dict: dict[str, torch.Tensor], mask_dict: dict[str, torch.Tensor]
    ):
