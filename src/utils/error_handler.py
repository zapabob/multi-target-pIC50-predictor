...
class ProductionErrorHandler:
    ...
    def _try_recovery(
        self, error: Exception, context: ErrorContext | None, retry_count: int, max_retries: int
    ) -> dict[str, Any]:
        ...
        if error_type in self.recovery_strategies:
            try:
                return self.recovery_strategies[error_type](
                    error, context, retry_count, max_retries
                )
            except Exception as recovery_error:
                self.logger.log_error(
                    recovery_error, context={"recovery_failed": True, "original_error": str(error)}
                )
                return {
                    "attempted": True,
                    "successful": False,
    ...
    def _handle_file_not_found(
        ...
        if context and context.model_name:
            self.logger.get_logger().warning(
                "Model file not found", model_name=context.model_name, action="attempting_download"
            )
    ...
    def _handle_validation_error(
        self, error: ValueError, context: ErrorContext | None, retry_count: int, max_retries: int
    ) -> dict[str, Any]:
        """Handle validation errors."""

        # Validation errors usually shouldn't be retried
        self.logger.get_logger().warning(
            "Validation error", error_message=str(error), action="no_retry_recommended"
        )
    ...
    **kwargs,
    ...
    try:
        ...
        error_handler_instance.handle_error(
            error=e, context=context, severity=severity, category=category
        )
