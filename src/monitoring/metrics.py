...
class MetricsCollector:
    ...
    def _setup_metrics(self) -> None:
        ...
        # Application metrics
        self.app_info = Info(
            "dat_predictor_info", "Application information", registry=self.registry
        )
        self.app_info.info(
            {"version": self.settings.app.version, "environment": self.settings.app.environment}
        )
        ...
        self.txgemma_tokens = Histogram(
            "dat_predictor_txgemma_tokens", "Number of tokens processed", registry=self.registry
        )

        # System metrics
        self.cpu_usage = Gauge(
            "dat_predictor_cpu_usage_percent", "CPU usage percentage", registry=self.registry
        )

        self.memory_usage = Gauge(
            "dat_predictor_memory_usage_percent", "Memory usage percentage", registry=self.registry
        )
        ...
        self.disk_usage = Gauge(
            "dat_predictor_disk_usage_percent", "Disk usage percentage", registry=self.registry
        )
    ...
    def record_prediction(
        ...
        self.predictions_total.labels(target=target, model_type=model_type, status=status).inc()

        self.prediction_duration.labels(target=target, model_type=model_type).observe(duration)

        if accuracy is not None:
            self.prediction_accuracy.labels(target=target, model_type=model_type).observe(accuracy)
    ...
    def record_training(
        ...
        self.training_duration.labels(target=target, model_type=model_type).observe(duration)

        self.training_loss.labels(
            target=target, model_type=model_type, epoch=str(epoch) if epoch is not None else "final"
        ).observe(final_loss)

        self.training_r2.labels(target=target, model_type=model_type).observe(val_r2)

    def record_txgemma_request(
        self, duration: float, token_count: int | None = None, status: str = "success"
    ) -> None:
    ...
    def record_active_learning_suggestion(self, strategy: str, target: str) -> None:
        """Record active learning suggestion."""

        self.active_learning_suggestions.labels(strategy=strategy, target=target).inc()
...
class MetricsMiddleware:
    ...
    def record_prediction_request(
        self, target: str, model_type: str, duration: float, status: str = "success"
    ) -> None:
        """Record prediction request metrics."""

        self.metrics_collector.record_prediction(
            target=target, model_type=model_type, duration=duration, status=status
        )

    def record_training_request(
        self, target: str, model_type: str, duration: float, final_loss: float, val_r2: float
    ) -> None:
